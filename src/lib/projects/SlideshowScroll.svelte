<script>
    ...
    function showSlide(n) {
        slideIndex = n;
        if (slideIndex > belgium.length) slideIndex = 1;
        if (slideIndex < 1) slideIndex = belgium.length;
    }
    ...
    $dericed: startAutoScroll();
</script>

<div 
    class="slideshow-container mx-auto max-w-screen-lg" 
    onmouseenter={stopAutoScroll} 
    onmouseleave={startAutoScroll}
    onfocus={stopAutoScroll}
    onblur={startAutoScroll}
    role="region"
    aria-label="Diaporama d'images"
    >
    <div class="relative">
        {#each belgium as slide, index (slide.src)}
            <div class="slides fade" class:hidden={index + 1 !== slideIndex}>
                <div class="nbr-text absolute w-10 h-10 bg-(--info-5) text-white font-bold rounded-full flex justify-center items-center m-2 right-0">{index + 1} / {belgium.length}</div>
                <img src={slide.src} alt={slide.caption} class="w-full" loading="lazy"/>
